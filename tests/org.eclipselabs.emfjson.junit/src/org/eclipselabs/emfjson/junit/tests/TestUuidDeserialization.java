...
public class TestUuidDeserialization extends UuidSupport {
	...
	public void setUp() {
		...
		mapper.configure(EMFJs.OPTION_INDENT_OUTPUT, true);
		mapper.configure(EMFJs.OPTION_USE_UUID, true);
	}
	...
	public void testDeserializeOneObject() {
		...
		ObjectNode node = mapper.to(root);
		EObject result = mapper.from(node, createUuidResource("result.xmi"));
